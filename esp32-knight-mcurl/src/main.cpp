...
uint32_t FileSize = 0;
const char *server_ascii;
char tenant_ascii[32];
char device_target_id_ascii[32];
const char *device_security_token_ascii;
...
String splitString(String &source, const String &delimiter) {
  int delimIndex = source.indexOf(delimiter);
  if (delimIndex == -1) {
    String token = source;
    source = "";
    return token;
  }
  String token = source.substring(0, delimIndex);
  source = source.substring(delimIndex + delimiter.length());
  return token;
}
